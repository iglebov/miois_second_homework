...
function test_efficiency()
    sizes = [100, 200, 500, 1000, 1500, 2000, 4000, 8000]
    ...
    for (idx, n) in enumerate(sizes)
        ...
        versions = [
            ("Threads (внешний)", cholesky_threads_outer_safe, L_serial),
            ("Threads (внутренний)", cholesky_threads_inner_safe, L_serial),
            ("Polyester (внешний)", cholesky_polyester_outer_safe, L_serial),
            ("Polyester (внутренний)", cholesky_polyester_inner_safe, L_serial)
        ]
        
        for (name, func, ref) in versions
            L_test = func(A)
